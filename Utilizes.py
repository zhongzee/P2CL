...
def Target_IntraClass_sim_loss(h_t, pred_t, source_mean, Top_n = 0):  # h_t表示目标域在流行上的特征矩阵。 source_mean表示源域类中心
    norm_h = h_t.pow(2).data.sum(1).pow(1/2).unsqueeze(1)
    norm_s = source_mean.pow(2).data.sum(1).pow(1/2).unsqueeze(1)
    h_t = h_t.mul(1/norm_h)  # 对特征矩阵的数据进行了处理
    source_mean = source_mean.mul(1/norm_s)  # 对源域类中心数据进行了处理
