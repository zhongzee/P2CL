...
optimizer_classifier_auxiliary = OptimWithSheduler(
    optim.SGD(classifier_auxiliary.parameters(), lr=args.train.lr / 10.0, weight_decay=args.train.weight_decay,
              momentum=args.train.momentum, nesterov=True),
    scheduler)
...
def train_model(args, totalNet, source_train_dl, target_train_dl, feature_extractor, feature_extractor_clip, classifier, discriminator,classifier_auxiliary, log_dir, output_device):
    # logger.add_graph(totalNet, torch.ones(2, 3, 224, 224))
    ...
    logger = SummaryWriter(log_dir)
    para_txt = open(join(log_dir, 'param.txt'), 'w')
    log_text = open(join(log_dir, 'log.txt'), 'w')
    log_wei = open(join(log_dir, 'log_wei.txt'), 'w')
    log_train = open(join(log_dir, 'log_train.txt'), 'w')
    model_dict = {
    ...
    while global_step < args.train.min_step:
        ...
        for i, ((im_source, label_source), (im_target, label_target)) in enumerate(iters):
            ...
            # 计算加权特征对齐损失
            feature_alignment_loss = (feature_alignment_loss_source * source_weights * weight).mean() + \
                                     (feature_alignment_loss_target * target_weights * weight).mean()
            ...
            with OptimizerManager(
                ...
                loss = ce + args.train.adv_loss_tradeoff * adv_loss + args.train.entropy_tradeoff * entropy + \
                       args.train.adv_loss_aug_tradeoff * adv_loss_aug + args.train.ce_aug_tradeoff * ce_aug + args.train.Source_Discri_weight * Source_Discri + args.train.clip_weight*feature_alignment_loss
            ...
            if global_step % args.log.log_interval == 0:
                ...
                logger.add_scalar('feature_alignment_source', feature_alignment_loss_target, global_step)
                logger.add_scalar('feature_alignment_target', feature_alignment_loss_source, global_step)
...
if __name__ == "__main__":
    ...
    # Define the dictionary to store trial results
    trial_results = {}
    # Define the objective function for Optuna optimization
    def objective(trial):
        # Define the hyperparameter search space
        # hyperparameters = {
        #     'Grass_lambda': trial.suggest_int('Grass_lambda', 100, 10000),
        #     'adv_loss_tradeoff': trial.suggest_float('adv_loss_tradeoff', 0.1, 2.0),
        #     'ce_aug_tradeoff': trial.suggest_float('ce_aug_tradeoff', 0.1, 2.0),
        #     'lr': trial.suggest_float('lr', 0.0001, 0.01),
        #     'entropy_tradeoff': trial.suggest_float('entropy_tradeoff', 0.1, 2.0),
        #     'min_step': trial.suggest_int('min_step', 100, 5000),
        #     'momentum': trial.suggest_float('momentum', 0.7, 0.99),
        #     'weight_decay': trial.suggest_float('weight_decay', 0.0001, 0.01),
        #     'adv_loss_aug_tradeoff': trial.suggest_float('adv_loss_aug_tradeoff', 0.1, 2.0),
        #     'Source_Discri_weight': trial.suggest_float('Source_Discri_weight', 0.1, 2.0),
        #     'clip_weight': trial.suggest_float('clip_weight', 0.1, 5.0)
        # }

        args['train']['adv_loss_tradeoff'] = trial.suggest_float('adv_loss_tradeoff', 0.5, 2.0)
        args['train']['ce_aug_tradeoff'] = trial.suggest_float('ce_aug_tradeoff', 0.5, 2.0)
        args['train']['lr'] = trial.suggest_float('lr', 0.0005, 0.002)
        args['train']['entropy_tradeoff'] = trial.suggest_float('entropy_tradeoff', 0.5, 2.0)
        # args['train']['min_step'] = trial.suggest_int('min_step', 1000, 2000)
        args['train']['min_step'] = 500
        # args['train']['momentum'] = trial.suggest_float('momentum', 0.7, 0.99)
        # args['train']['weight_decay'] = trial.suggest_float('weight_decay', 0.0001, 0.01)
        args['train']['adv_loss_aug_tradeoff'] = trial.suggest_float('adv_loss_aug_tradeoff', 0.5, 5.0)
        args['train']['Source_Discri_weight'] = trial.suggest_float('Source_Discri_weight', 0.5, 5.0)
        args['train']['clip_weight'] = trial.suggest_float('clip_weight', 0.5, 5.0)


        print("args['train']['adv_loss_tradeoff'] =", args['train']['adv_loss_tradeoff'])
        print("args['train']['ce_aug_tradeoff'] =", args['train']['ce_aug_tradeoff'])
        print("args['train']['lr'] =", args['train']['lr'])
        print("args['train']['entropy_tradeoff'] =", args['train']['entropy_tradeoff'])
        print("args['train']['min_step'] =", args['train']['min_step'])
        print("args['train']['momentum'] =", args['train']['momentum'])
        print("args['train']['weight_decay'] =", args['train']['weight_decay'])
        print("args['train']['adv_loss_aug_tradeoff'] =", args['train']['adv_loss_aug_tradeoff'])
        print("args['train']['Source_Discri_weight'] =", args['train']['Source_Discri_weight'])
        print("args['train']['clip_weight'] =", args['train']['clip_weight'])

        # txt_source_label 不是一个超参数，所以它应该不在这里被设置
        ...
        # acc_test = train_model(args, totalNet, source_train_dl, target_train_dl, log_dir,
        #                        output_device,logger)
        acc_test = train_model(args, totalNet, source_train_dl, target_train_dl, feature_extractor, feature_extractor_clip, classifier, discriminator,classifier_auxiliary, log_dir, output_device)

        # Store trial results
        trial_id = trial.number
        trial_results[trial_id] = {
            'hyperparameters': args['train'],
            'acc_test': acc_test
        }

        results_filepath_curr = '/root/BCD_PDA/trial_results_curr'+str(trial_id)+'.json'  # Change this path as needed for your environment
        with open(results_filepath_curr, 'w') as f:
